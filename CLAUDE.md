...
### Nota: Manuales (docs/)
- **docs/generate-manual.cjs** → genera `Manual de Usuario - Sistema POS La Economía.docx` (guía operativa por rol: Cajero, Repositor, Administrador). Ya existía; no estaba referenciado acá.
- **docs/generate-technical-manual.cjs** → genera `Manual Técnico - Sistema POS La Economía.docx` (arquitectura, modelo de datos, RPCs, lógica de negocio, deploy). Usa la librería `docx` (no está en `package.json`, instalar con `npm install --no-save docx` para regenerar) e incrusta 6 diagramas PNG (arquitectura, estructura de módulos, modelo de datos, flujo de auth, flujo de venta, flujo de compras) construidos a mano en SVG y rasterizados con `@resvg/resvg-js`. Documentó en su momento que la tabla `customer_payments` estaba en uso pero no versionada en `supabase/schema.sql` — ver nota siguiente, ya corregido.
- Ambos scripts son generadores puntuales (`node docs/generate-*.cjs`), no forman parte del build de la app.

### Nota: Cierre de materia — INSTALL.md + suite de tests + fix schema.sql
- **supabase/schema.sql** — se agrega la tabla `customer_payments` (créditos de cuenta corriente, usada por `customerService.registerPayment`/`getAccountStatement`) que existía en producción pero nunca había sido versionada; incluye su RLS + políticas igual que el resto de las tablas.
- **INSTALL.md** (raíz) — guía de instalación desde cero en un ambiente nuevo e independiente: requisitos, clonar, `.env`, orden exacto de los 4 SQL a correr en Supabase, setup inicial, seed opcional, build/deploy en Vercel, impresora térmica, backup (`pg_dump`, no hay backup automático en el plan free), mantenimiento sugerido, cómo correr los tests (sección 14 — sin archivo aparte, a pedido explícito: no separar la doc de tests en su propio README), troubleshooting.
- **tests/** (nuevo) — primera suite de tests del proyecto, con Vitest + Testing Library (sí quedaron en `package.json`, a diferencia de `docx`/`resvg`). `npm test` corre todo. Estructura: `unit/` (formatters, useCartStore, ticketPrinter, authService), `integration/` (createSale, cuenta corriente, turno de caja — con Supabase mockeado vía `tests/helpers/supabaseMock.js`), `components/` (LoginForm, CustomerFormModal, ProductSearch, con Testing Library), `performance/` (carrito con ~500 ítems). Requiere `vitest.config.js` (alias `@` propio, no toca `vite.config.js`) y `.env.test` (valores dummy, sin secretos, para que `supabase/client.js` no explote al importarse).
- 76 tests, 11 archivos, todos pasando al momento de escribir esto.
